...
ON F.facid = B.facid
WHERE LEFT( F.name, 12 ) =  'Tennis Court'
ORDER BY membername
...
SELECT sub.membername,
	name AS facilityname,
	CASE WHEN sub.firstname = 'GUEST' THEN F.guestcost
...
FROM (
 ...
 FROM country_club.Members M 
	...
	ON M.memid = B.memid
	WHERE LEFT(B.starttime, 10) = '2012-09-14') AS sub
INNER JOIN country_club.Facilities F
...
SELECT name,
SUM(CASE WHEN sub.firstname = 'GUEST' THEN F.guestcost 
